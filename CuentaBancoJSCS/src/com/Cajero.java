...
//	Scanner sc = new Scanner(System.in);
	...
	public Object[] buscar(String nombreUser, String nip) {
		...
		for (int i = 0; i < cuentas.size(); i++) {
			...
			if ((cuentas.get(i).getUser().getNombre().equals(nombreUser)|| cuentas.get(i).getNumeroCuenta().contentEquals(nombreUser)) && cuentas.get(i).getNip().equals(nip)) {
	...
	private String digitos(Cuenta cuenta) {
		return cuenta.getNumeroCuenta().substring(cuenta.getNumeroCuenta().length()-4, cuenta.getNumeroCuenta().length());
	}
	...
	@Override
	public Object transferir(double monto, String cuentaOrigen, String nipCuentaO, String cuentaDestino, String nipCuentaD) {
		Ticket ticket = null;
		
		if (this.buscar(cuentaOrigen, nipCuentaO)[1] != null && this.buscar(cuentaDestino, nipCuentaD)[1] != null)  {
			Cuenta cuentaO = (Cuenta) this.buscar(cuentaOrigen, nipCuentaO)[1];
			Cuenta cuentaD = (Cuenta) this.buscar(cuentaDestino, nipCuentaD)[1];
			int indexO = (int) this.buscar(cuentaOrigen, nipCuentaO)[0];
			int indexD = (int) this.buscar(cuentaDestino, nipCuentaD)[0];
